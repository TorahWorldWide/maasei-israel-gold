    ...
    <section className="relative overflow-hidden" style={{ background: "radial-gradient(ellipse at 50% 0%, #1a1505 0%, #0d0d0d 55%, #0a0a0a 100%)" }}>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 pt-8 pb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_minmax(0,2.3fr)_1fr] gap-4 md:gap-5 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 mb-2" style={{ color: GOLD }}>
              <span className="text-2xl leading-none" aria-hidden="true">✦</span>
              <span className="text-xs font-semibold tracking-wide">חלק א׳ · הניצוץ</span>
            </div>
            <p className="text-base md:text-lg font-bold leading-relaxed" style={{ color: "#f5f0e6" }}>
              {spark}
            </p>
            <div className="mt-3 flex items-center gap-2 justify-center md:justify-start">
              <span className="text-[11px] px-2.5 py-0.5 rounded-full" style={{ color: GOLD, background: "rgba(212,175,55,0.12)", border: "1px solid rgba(212,175,55,0.3)" }}>
                {current.category}
              </span>
              {current.year && <span className="text-white/45 text-xs">{current.year}</span>}
            </div>
          </aside>
          ...
          >
            <div className="inline-flex items-center gap-2 mb-2" style={{ color: "#ffd966" }}>
              <span className="text-2xl leading-none" aria-hidden="true">☀</span>
              <span className="text-xs font-semibold tracking-wide">חלק ב׳ · האור</span>
            </div>
            <p className="text-sm md:text-base leading-relaxed line-clamp-6 md:line-clamp-none" style={{ color: "rgba(245,240,230,0.9)" }}>
              {light}
            </p>
            <a
